...
function calculateTorinoScale(diameterKm: number, energyMegatons: number, isPHA: boolean, isSentry: boolean): number {
    if (isSentry) {
...
function calculateCraterSize(diameterKm: number, velocityKmS: number): number {
    const craterDiameterKm = diameterKm * 20 * Math.pow(velocityKmS / 20, 0.33);
...
function calculateImportanceScore(diameterKm: number, velocityKmS: number, missDistanceAU: number, isPHA: boolean, isSentry: boolean): number {
    let score = 0;
...
export default function App(): JSX.Element {
        ...
        const fetchAsteroidData = async () => {
            ...
            try {
                ...
                const enrichedData: AsteroidData[] = data.map(asteroid => {
                    ...
                    const importanceScore = calculateImportanceScore(diameterKm, velocityKmS, missDistanceAU, isPHA, isSentry);
    ...
    if (isLoadingAsteroids) {
        return (
            <div className="flex h-screen items-center justify-center" style={{ backgroundColor: '#051622' }}>
                <div className="text-center">
                    <div className="w-12 h-12 border-4 border-t-transparent rounded-full animate-spin mx-auto mb-4" style={{ borderColor: '#1ba098' }}></div>
                    <h2 className="text-2xl font-light mb-2" style={{ color: '#1ba098' }}>Loading Asteroid Data</h2>
                    <p className="text-sm opacity-70" style={{ color: '#deb992' }}>Fetching from database...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="flex h-screen items-center justify-center" style={{ backgroundColor: '#051622' }}>
                <div className="text-center max-w-md">
                    ...
                    <h2 className="text-2xl font-light mb-4" style={{ color: '#1ba098' }}>Failed to Load Data</h2>
                    <p className="text-sm mb-4" style={{ color: '#deb992' }}>{loadError}</p>
                    <button
        ...
        return (
            <div className="flex h-screen items-center justify-center" style={{ backgroundColor: '#051622' }}>
                <div className="text-center">
                    ...
                    <h2 className="text-2xl font-light mb-4" style={{ color: '#1ba098' }}>No Asteroid Data</h2>
                    <p className="text-sm" style={{ color: '#deb992' }}>No asteroids found in database</p>
                </div>
            </div>
        );
    }
