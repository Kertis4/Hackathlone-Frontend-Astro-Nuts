...
export default function App(): JSX.Element {
  ...
  useEffect(() => {
    ...
    visibleAsteroids.forEach((asteroid, index) => {
      ...
      console.log(`Asteroid ${asteroid.name} - Size: ${size.toFixed(2)} at distance: ${distance}`);
    ...
    <div className="flex h-screen bg-gray-900 text-white">
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-sm mb-1">
                ...
                <span className="text-xs text-gray-400 block">Ranked by importance (Torino Scale)</span>
              </label>
              <input
                type="range"
                min="1"
                ...
                step="1"
                value={maxAsteroids}
                onChange={(e) => setMaxAsteroids(parseInt(e.target.value))}
                className="w-full"
              />
        ...
        <div className="absolute top-4 right-4 bg-gray-800 bg-opacity-90 backdrop-blur-sm rounded-lg p-4">
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className="text-xs text-gray-400 mt-2 pt-2 border-t border-gray-600">
              ✨ 15,000 procedural stars<br/>
              🌍 Simplified continent outlines<br/>
      ...
      <div className="w-96 bg-gray-800 bg-opacity-95 backdrop-blur-sm p-6 overflow-y-auto">
        ...
        {selectedAsteroid ? (
          <div className="space-y-4">
            <div className="bg-gray-700 bg-opacity-80 rounded-lg p-4">
              ...
              <div className="flex items-center space-x-2 mb-2">
                <div className={`inline-block px-2 py-1 rounded text-xs font-bold ${getRiskColor(getRiskLevel(selectedAsteroid))}`}>
              ...
              <div className="mb-2">
                ...
                <span className="font-mono text-yellow-400">{selectedAsteroid.importance_score}/10</span>
              </div>
              <a 
            ...
            <div className="bg-gray-700 bg-opacity-80 rounded-lg p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  ...
                  <p className="font-mono">{formatNumber(selectedAsteroid.estimated_diameter_km_max)}</p>
                </div>
                <div>
                  <span className="text-gray-400">Magnitude:</span>
                  <p className="font-mono">{selectedAsteroid.absolute_magnitude_h}</p>
                </div>
                <div>
                  <span className="text-gray-400">Velocity:</span>
                  <p className="font-mono">{formatNumber(selectedAsteroid.relative_velocity_km_s)} km/s</p>
                </div>
                <div>
                  <span className="text-gray-400">Distance (AU):</span>
                  <p className="font-mono">{formatNumber(selectedAsteroid.miss_distance_au)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-700 bg-opacity-80 rounded-lg p-4 space-y-3">
              ...
              <div className="space-y-2 text-sm">
                <div>
                  ...
                  <p className="font-mono text-red-400">{formatNumber(selectedAsteroid.impact.energy_megatons)} Megatons TNT</p>
                </div>
                <div>
                  <span className="text-gray-400">Crater Size:</span>
                  <p className="font-mono">{formatNumber(selectedAsteroid.impact.crater_km)} km diameter</p>
                </div>
                <div>
                  <span className="text-gray-400">Risk Zones:</span>
                  <div className="mt-1">
                    {selectedAsteroid.impact.risk_zones.map((zone, index) => (
                      <span key={index} className="inline-block bg-red-900 bg-opacity-80 text-red-200 px-2 py-1 rounded text-xs mr-1 mb-1">
                        {zone}
                      </span>
                    ))}
                  </div>
            ...
            <div className="bg-gray-700 bg-opacity-80 rounded-lg p-4">
              ...
              <div className="space-y-2">
                <button 
                  onClick={() => alert(`For ${selectedAsteroid.name}: Kinetic Impactor mission would redirect using spacecraft impact`)}
                ...
                <button 
                  onClick={() => alert(`For ${selectedAsteroid.name}: Gravity Tractor would use gravitational pull to slowly alter orbit`)}
                  className="w-full bg-purple-600 bg-opacity-80 hover:bg-purple-700 px-4 py-2 rounded text-sm transition-colors"
                >
                  Gravity Tractor
                </button>
                <button 
                  onClick={() => alert(`For ${selectedAsteroid.name}: Nuclear deflection would use controlled explosion to change trajectory`)}
          ...
          <div className="space-y-4">
            <p className="text-gray-400">Click on an asteroid in the visualization to view detailed analysis</p>
